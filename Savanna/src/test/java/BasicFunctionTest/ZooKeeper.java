...
public class ZooKeeper {
	...
	public void case1_ServiceStop() throws Exception {
		...
		// 서비스 중지  
		service.stop(Service.ZooKeeper);
		
		// 요약 페이지 문구 변경 확인
		service.checkStatus(Component.ZooKeeperServer, ServiceStatus.Stoped, driver);
		
		// 프로세스 kill 확인
		List<String> hosts = service.getHost(Component.ZooKeeperServer);
	...
	public void case2_ServiceStart() throws Exception {
		...
		// 서비스 시작  
		service.start(Service.ZooKeeper);
		
		// 요약 페이지 문구 변경 확인
		service.checkStatus(Component.ZooKeeperServer, ServiceStatus.Started, driver);
